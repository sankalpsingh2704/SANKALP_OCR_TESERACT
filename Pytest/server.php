<?php
	...
	$out_shell = shell_exec("sh ocrthepdfoptimized.sh " .$target_Path);
	#echo $out_shell;
  ...
  file_put_contents($downloadfile, implode("\n", $rows));
	...
	//print_r($nameslist);
	$blacklist = array("INVOICE","TAX","PRIVATE","LIMITED","VENDOR","PACKAGING","GREEN","FICUS","CUM","CHALLAN","&","TIRUMALA","PVT","LTD","SYSTEMS","AO","WORKS","TIMBER","INDUSTRIES","SHEET","CORPORATION","ANEKAL","CO.","SUPER");
	...
	foreach($nameslist as $v){
		...
		$sql = "SELECT templid, templname, templfields , templinvname FROM templatemanager where templname like '%".$v."%'";
		...
		while($row = $result->fetch_assoc())
		{
			...
			$templateId = $row["templid"];
			#echo "template:".$templname."TempID:".$templateId;
			break;
		}
		break;
	}
	...
	$out_shell = "";
	//$command = escapeshellcmd("/usr/bin/python3 regular.py /var/www/html/iqinvoiceapigst/pytest/uploads/51618.pdf.xml 8");
	$command = escapeshellcmd("/usr/bin/python3 regular.py uploads/".$file_name .".xml ".$templateId);
	//$command = escapeshellcmd("/usr/bin/python3 test.py");
	//echo $command;
	$out_shell = shell_exec($command);

	
	#echo $file_name;
	//echo $out_shell;
	$pyfields = explode("::",$out_shell);
	
	
	
	array_push($pyfields,$templname);
	print_r($pyfields);
	$fieldsname = array("INVOICENO","PAN","GSTIN","PO","INVOICEDATE","AMOUNT","VENDOR");
	...
	for($i = 0;$i < sizeof($fieldsname); $i++){
		$response .= "<".$fieldsname[$i].">".$pyfields[$i]."</".$fieldsname[$i].">" ;
	}
	...
	for($i = 1;$i < sizeof($fields_collection); $i++)
	{
		...
		{
			if($fieldsname[$i-1] == "VENDOR"){
				$response .= "<".$fieldsname[$i-1].">".$templname."</".$fieldsname[$i-1].">";
				}
			else
			{	
			...
			while($row = $result->fetch_assoc())
			{
				...
				for($j = 0; $j < sizeof($dataarea); $j++)
				{
					...
					if(($row["datafield2"] - 10) <= $dataarea[$j]['t'] && ($row["datafield3"]- 10) <= $dataarea[$j]['l'] && ($row["datafield2"] + 10) >= $dataarea[$j]['t'] && ($row["datafield3"] + 5) >= $dataarea[$j]['l'] && $dataarea[$j]['l'] >= ($row["datafield3"] - 5) )
					{
	...
	function searchId($array,$dataarea)
	{
		for($i = 0; $i < sizeof($dataarea); $i++){
                        //$array['t'] == $dataarea[$i]['t'] && $array['l'] == $dataarea[$i]['l'] && $array['w'] == $dataarea[$i]['w'] && $array['h'] == $dataarea[$i]['h'] && $array['f'] == $dataarea[$i]['f']
			if($array['t'] +20 >= $dataarea[$i]['t'] && $dataarea[$i]['t'] >= $array['t'] - 20)
			{
