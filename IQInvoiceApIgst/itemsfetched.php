<?php
  ...
  file_put_contents($downloadfile, implode("\n", $rows));
	...
	print_r($nameslist);
	$blacklist = array("INVOICE","TAX","PRIVATE","LIMITED","VENDOR","PACKAGING","GREEN","FICUS","CUM","CHALLAN","&","PVT","LTD","WORKS","TIMBER","INDUSTRIES","SHEET","PALLET","WOODEN","CORPORATION","ANEKAL");
		...
		if($result->num_rows > 0)
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				for($j = 0; $j < sizeof($dataarea); $j++)
				{
					...
					if(($row["datafield2"] - 10) <= $dataarea[$j]['t'] && ($row["datafield3"]-10) <= $dataarea[$j]['l'] && ($row["datafield2"] + 10) >= $dataarea[$j]['t'] && ($row["datafield3"] + 10) >= $dataarea[$j]['l'] )
					{
	...
	function searchId($array,$dataarea)
	{
		for($i = 0; $i < sizeof($dataarea); $i++){
			...
			//$array['t'] == $dataarea[$i]['t'] && $array['l'] == $dataarea[$i]['l'] && $array['w'] == $dataarea[$i]['w'] && $array['h'] == $dataarea[$i]['h'] && $array['f'] == $dataarea[$i]['f'] , $array['l'] + 5 >= $dataarea[$i]['l'] , $array['w'] + 5 >= $dataarea[$i]['w'] && $dataarea[$i]['w'] >= $array['w'] - 5
			
			if( $array['t'] +20 >= $dataarea[$i]['t'] && $dataarea[$i]['t'] >= $array['t'] - 20)
			{
				echo "<br/> ID".$dataarea[$i]['id'];
